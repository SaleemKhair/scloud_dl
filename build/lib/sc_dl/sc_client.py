...
class SCClient(RetryClient):
    def __init__(self, config: Configurations):
        ...
        self._headers = {
            'Autherization': f'OAuth {self._config.credentials.oauth_token}',
            'Accept': 'application/json'
        }

    async def _get_collection(self, target):

        resp = await self.get(
            url=target,
            headers=self._headers,
            params={
                'client_id': f'{self._config.credentials.client_id}',
    ...
    async def _from_json(self, json_track) -> Track:
        if len(json_track['media']['transcodings']) > 0:
            resp = await self.get(
                url=json_track['media']['transcodings'][0]['url'],
                params={'client_id': f'{self._config.credentials.client_id}'},
                headers=self._headers
            )
    ...
    async def get_tracks(self):
        ...
        target = f"/users/{self._config.credentials.user_id}/{self._config.collection_type}"
        tasks = set()
        next_href = f'{self._config.base_url}{target}'

        async def _populate_tasks(href):
            (liked_collection, next_href) = await self._get_collection(href)
            for item in liked_collection:
                ...
                task = self._from_json(json_track)
                tasks.add(task)
